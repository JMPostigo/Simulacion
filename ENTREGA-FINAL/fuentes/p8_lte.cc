...
*       export 'NS_LOG=p8_lte=level_debug'
*
...
double RealizaSimulacion(double t_simulacion, double posicion_UE, double radio_cobertura_eNB,
  ...
  450 metros (con cobertura asegurada de 300-600 metros), y así sucesivamente, para cada
  caso a simular. */
  ...
  // En nuestro escenario, los nodos estarán fijos en una posición, para cada simulación.
  ...
  // Instalamos la pila TCP/IP en el nodo UE, y le asignamos dirección IP.
  ...
  Tras ello, fijamos el nodo UE al último nodo eNB instalado (para que avance en el tramo
  a cubrir). */

  Ptr<Node> nodoUE = nodosUE.Get (0);
  Ptr<Ipv4StaticRouting> rutaEstaticaUE = 
                         ipv4RoutingHelper.GetStaticRouting (nodoUE->GetObject<Ipv4> ());
  ...
  startTimeSeconds->SetAttribute ("Min", DoubleValue (MIN_INICIO_APPS));
  startTimeSeconds->SetAttribute ("Max", DoubleValue (MAX_INICIO_APPS));
  ...
  /* Obtenemos el porcentaje de paquetes correctos en la variable resultado_sim, usando
  el método correspondiente de Observador. Esta variable se devolverá al método principal. */
  ...
  // Para acabar, reinicializamos los atributos del observador.
...
int main (int argc, char *argv[])
{
  ...
  con valor 3 de exponente de la fórmula de dicho modelo. Con estos valores, el radio de
  cobertura del nodo eNB que garantice el porcentaje de paquetes correctos definido anteriormente  
  estará en torno a 100-150 metros (considerando que el patrón de radiación de la antena es
  circular, correspondiente a una antena omnidireccional, como se verá en los resultados). */
  ...
  /* Esta variable, de tipo Average, contendrá la estadística que se obtiene de RealizaSimulacion.
  En concreto, el porcentaje de paquetes correctos. De esta forma, y realizando un número de 
  "iteraciones" simulaciones, computaremos la media de los valores obtenidos y hallaremos el 
  intervalo de confianza para cada punto de la gráfica a calcular. */

  Average<double> acum_porcentaje_correctos;
  acum_porcentaje_correctos.Reset();

  NS_LOG_INFO("Acumulador instanciado y reseteado.");

  /* En esta variable, se guardarán los distintos resultados de la simulación, que se
  irán añadiendo a la variable Average con un Update, para luego computar la media. */
  ...
  for (para cada una de las curvas) {
    
    Ponemos nombre y estilo a la curva que vamos a representar. 

    while (mientras sigamos dentro del tramo a cubrir) {

      for {
        Bucle en el que se realizan las "iteraciones" simulaciones. Después, se guardan
        los resultados en el acumulador, para computar la media posteriormente.    
      }

      if {
        Cuando el porcentaje de paquetes correctos, en la media calculada tras la
        simulación considerada, baje de un 40% (por ejemplo, para el primer caso), debemos 
        considerar, por calidad de servicio, el colocar un nuevo nodo eNB (recordar que 
        nuestro objetivo es garantizar que el porcentaje de paquetes correctos nunca bajará 
        de ese porcentaje). Notar que repetiríamos la simulación con el nodo UE colocado en 
        la misma posición, pero añadiendo otro nodo eNB más, conectándose el nodo UE a ese 
        nuevo nodo eNB.
      }
  ...
  for (uint16_t i = 0; i < NUM_CURVAS; i++) {
    ...
    de confianza. 
    La segunda curva, que es la de la evolución del número de antenas según se posiciona
    el nodo UE, no necesita de intervalo de confianza, ya que es una simple muestra del
    momento en el que se decide añadir un nodo eNB. */
